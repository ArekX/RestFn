...
Everything is configured through the array you pass to `WebApp::createDefault()`
(or to the container directly). This page is the reference for every value RestFn
reads. For *how* configuration is injected and resolved, see [Dependency
Injection](di.md).
...
`config.overrides[SomeClass::class]` only when one specific class needs a
different value than the global one. A class resolves each value as
**override → global → built-in default**.
...
[`WebApp::DEFAULT_ALIASES`](architecture.md#everything-is-swappable). `factories`
bind a class to a factory that builds it.
...
`createDefault()` fills `ops` with every built-in operation
(`WebApp::DEFAULT_OPS`) when you do not set it. Set it to a narrower map to
restrict what a client can use — it becomes an allow list.
...
A request is a client-supplied expression tree, so these limits bound how much
work one request can ask for. The defaults are safe; lower them if your clients
have no legitimate need for deep or large expressions.
...
(`ErrorMiddleware` then `AuthenticationMiddleware`) when you do not set it.
Setting it yourself replaces the whole stack, so include those two if you still
want them.

## Authentication

Read by the authentication services. See [Authentication](authentication.md) for
the full picture.
