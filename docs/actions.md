...
An action is your code. It is the part of a request that talks to your database,
calls another service, or does any real work. Operations like `value`, `get` and
`map` shape data, but they cannot do anything on their own — actions are how the
client reaches your logic.

Actions are similar to endpoints in a normal REST server, except that several of
them can be run in a single request.

## Defining an action

An action implements `ActionInterface`. It has one method, `run`, which receives
the data the client passed and returns an array:
...
The result must be an array. That array is what the client gets back, or what the
surrounding operations work on.

## Registering an action

Actions are called by name. You register the names the `run` operation can use
under `actions`:
...
Only registered actions can be run. If a client asks for an action that is not
registered, the request fails.

## Calling an action

The client calls an action with the `run` operation, passing the action name and
the data:
...
The second parameter is the data passed to `run($data)`. If it is an operation it
gets evaluated first, otherwise it is passed through as is. To pass an array or
object directly (so it is not treated as an operation), wrap it in `value`:
...
Actions are created through the container, so they can declare their dependencies
in the constructor and the container will inject them:
...
`AuthenticatedActionInterface`. It is a marker with no methods:
...
If this action is run without an identity the request is rejected. See
...
receives raw data, a list action receives a `ListRequest` and returns a
`ListResult`:
...
`ListRequest` gives you the page, page size, filters, and the properties the
client asked for. `ListResult` carries the total number of items and the rows for
the current page.
